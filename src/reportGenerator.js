...
function createMonthlyReportPDF(transactions, monthName, user) {
    return new Promise((resolve) => {
        ...
        const itemX = 50, dateX = 150, typeX = 250, amountX = 450;
        doc.fontSize(10).text('Date', itemX, tableTop).text('Description', dateX, tableTop).text('Type', typeX, tableTop).text(`Amount (${currency})`, amountX, tableTop, { align: 'right' });
        doc.moveTo(itemX, doc.y).lineTo(550, doc.y).stroke();
        doc.font('Helvetica');
        transactions.forEach(t => {
            const y = doc.y + 15;
            const formattedDate = t.createdAt.toLocaleDateString('en-GB');
            doc.fontSize(10).text(formattedDate, itemX, y).text(t.description, dateX, y).text(t.type.charAt(0).toUpperCase() + t.type.slice(1), typeX, y).text(t.amount.toLocaleString(), amountX, y, { align: 'right' });
...
function createInventoryReportPDF(products, logs, monthName, user) {
    return new Promise((resolve) => {
        ...
        products.forEach((product, index) => {
            ...
            doc.fontSize(11).font('Helvetica').text(`- Stock Status: ${product.stock} units remaining.`).text(`- Units Sold this month: ${unitsSold}`).text(`- Total Revenue: ${currency} ${revenue.toLocaleString()}`).text(`- Cost of Goods Sold: ${currency} ${costOfGoodsSold.toLocaleString()}`).font('Helvetica-Bold').text(`- Gross Profit: ${currency} ${profit.toLocaleString()}`).font('Helvetica');
            doc.moveDown();
            if (productLogs.length > 0) {
                const tableTop = doc.y;
                doc.fontSize(9).font('Helvetica-Bold').text('Date', 50, tableTop).text('Action', 150, tableTop).text('Quantity Change', 350, tableTop).text('Notes', 500, tableTop);
                doc.moveTo(50, doc.y).lineTo(740, doc.y).stroke();
                doc.font('Helvetica');
                productLogs.forEach(log => {
                    const y = doc.y + 12;
                    doc.fontSize(9).text(log.createdAt.toLocaleDateString('en-GB'), 50, y).text(log.type.charAt(0).toUpperCase() + log.type.slice(1), 150, y).text(log.quantityChange.toString(), 350, y).text(log.notes || '', 500, y);
                    doc.moveTo(50, doc.y + 10).lineTo(740, doc.y + 10).strokeColor('#dddddd').stroke();
                });
            }
            if (index < products.length - 1) doc.addPage({ margin: 50, layout: 'landscape' });
        });
        doc.end();
        const buffers = [];
        stream.on('data', chunk => buffers.push(chunk));
