...
// [FIXED] Draw Footer without creating new pages
const drawFooter = (doc) => {
    const range = doc.bufferedPageRange(); // Gets { start: 0, count: X }
    ...
    for (let i = 0; i < range.count; i++) {
        ...
        // 2. DISABLE MARGINS: This prevents the "New Page" trigger when writing at the bottom
        ...
        // Left Text (Generated by...)
        doc.fillColor('#999999').fontSize(8)
           .text('Generated by Fynax', 50, doc.page.height - 40, { 
               align: 'left',
               width: 200,
               lineBreak: false 
           });
        ...
        // We use absolute positioning (x=350) to ensure it aligns right correctly
        doc.text(`Page ${i + 1} of ${range.count}`, 350, doc.page.height - 40, { 
               align: 'right',
               width: 200,
               lineBreak: false 
           });

        // 3. RESTORE STATE (Safety for future edits)
        doc.restore();
    }
};
