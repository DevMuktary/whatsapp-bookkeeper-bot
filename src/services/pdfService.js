...
// Helper for Date & Time (DD/MM/YYYY, HH:MM AM/PM)
...
// [UPDATED] Footer now contains the Timestamp to reduce header clutter
const drawFooter = (doc) => {
    const range = doc.bufferedPageRange(); 
    const timestamp = formatDateTime(new Date()); // Capture time once for consistent footer
    ...
    for (let i = 0; i < range.count; i++) {
        ...
        // Center: Timestamp [NEW PROFESSIONAL LOCATION]
...
const drawHeader = (doc, user, title, period) => {
    ...
    // Reduced height back to 30 since we removed the timestamp line
    ...
    // [REMOVED] Timestamp from here to avoid confusion

    doc.strokeColor(COLORS.accent)
       .lineWidth(2)
       .moveTo(50, 80) // Moved up slightly since header is smaller
       .lineTo(550, 80)
       .stroke();
...
export function generateInventoryReport(user, products) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Inventory report "Period" is just "As of Today"
