 ...
 * This is the new "brain" of the bot.
 */
async function _processIncomingMessage(message, collections) {
    ...
    if (!user) {
        ...
        const newConversation = { userId: senderId, state: 'awaiting_store_name', history: [] };
    ...
    if (messageText.startsWith('/') && user.role === 'admin') {
        // NOTE: adminService will be updated in the next step to use the new whatsappService
    ...
    if (conversation?.chatState === 'live') {
        // NOTE: liveChatService will be updated in the next step
    ...
    if (conversation?.state) {
        // NOTE: onboardingService will be updated in the next step
    ...
    // NOTE: aiService will be updated in the next step
    await aiService.processMessageWithAI(messageText, collections, senderId, user);
}
