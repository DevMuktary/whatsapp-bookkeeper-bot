...
// [NEW] Currency to Timezone Mapping
// If a user's currency matches, we use that region's time.
// If not found, we default to 'Africa/Lagos'.
const TIMEZONE_MAP = {
    'NGN': 'Africa/Lagos',
...
// Helper to determine timezone
const getTimeZone = (currency) => {
    return TIMEZONE_MAP[currency] || 'Africa/Lagos';
};
...
// [UPDATED] Dynamic Date & Time Formatter
const formatDateTime = (dateInput, timeZone) => {
    if (!dateInput) return '';
...
// [UPDATED] Dynamic Date Formatter
...
// [UPDATED] Footer now accepts Timezone to print correct time
const drawFooter = (doc, timeZone) => {
    const range = doc.bufferedPageRange(); 
    const timestamp = formatDateTime(new Date(), timeZone); // Capture dynamic time
    ...
    for (let i = 0; i < range.count; i++) {
        ...
        // Left: Branding
        doc.text('Generated by Fynax', 50, doc.page.height - 40, { align: 'left', width: 150, lineBreak: false });
        
        // Center: Timestamp (Moved to Footer as requested)
        doc.text(`Printed: ${timestamp}`, 200, doc.page.height - 40, { align: 'center', width: 200, lineBreak: false });

        // Right: Page Number
        doc.text(`Page ${i + 1} of ${range.count}`, 400, doc.page.height - 40, { align: 'right', width: 150, lineBreak: false });
...
const drawHeader = (doc, user, title, period) => {
    ...
    doc.rect(pageWidth - 250, 35, 200, 30).fill(COLORS.grayLight); 
    
    doc.fillColor(COLORS.text)
       .fontSize(12)
       .font(FONTS.bold)
       .text(title.toUpperCase(), pageWidth - 250, 38, { width: 200, align: 'center' });

    if (period) {
        doc.fontSize(10)
           .font(FONTS.regular)
           .text(period, pageWidth - 250, 52, { width: 200, align: 'center' });
    }
...
export function generateSalesReport(user, transactions, periodTitle) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const timeZone = getTimeZone(user.currency);
            ...
            transactions.forEach((tx, i) => {
                ...
                // Pass timeZone to formatDate
                drawTableRow(doc, currentY, [
                    { text: formatDate(tx.date, timeZone), ...colDate },
            ...
            // Pass timeZone to Footer
            drawFooter(doc, timeZone);
            doc.end();
        } catch (error) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            transactions.forEach((tx, i) => {
                ...
                drawTableRow(doc, currentY, [
                    { text: formatDate(tx.date, timeZone), ...cols[0] },
...
export function generateInventoryReport(user, products) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Inventory "As of" time uses dynamic timezone
            let currentY = drawHeader(doc, user, 'Inventory Report', `As of ${formatDateTime(new Date(), timeZone)}`);
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            items.forEach((item, i) => {
                ...
                drawTableRow(doc, currentY, [
                    { text: formatDate(item.date, timeZone), ...colDate },
...
export function generateInvoice(user, transaction, customer) {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.font(FONTS.regular).text(formatDate(transaction.date, timeZone), 460, y, { align: 'right', width: 80 });
