...
import { sendTextMessage, sendInteractiveButtons, sendInteractiveList, sendMainMenu, sendReportMenu, setTypingIndicator } from '../api/whatsappService.js'; // [FIXED] Removed markMessageAsRead
...
export async function handleMessage(message) {
  ...
  try {
    // [FIXED] Calls the combined function (Read + Typing)
...
async function handleIdleState(user, text) {
    ...
    if (intent === INTENTS.GENERAL_CONVERSATION) {
        // Use the reply generated by AI
        const aiReply = context.generatedReply || "I am your bookkeeping assistant. How can I help you today?";
        await sendTextMessage(user.whatsappId, aiReply);
        return;
    } 
    
    if (intent === INTENTS.CHITCHAT) {
        await sendTextMessage(user.whatsappId, "Hello! 👋 Ready to organize your finances? Let me know what you need.");
        return;
    }
