...
// Footer logic for Reports (not Invoices)
const drawFooter = (doc, timeZone) => {
    const range = doc.bufferedPageRange(); 
    const timestamp = formatDateTime(new Date(), timeZone); 
...
// [FIX] Updated Generate Invoice with Pagination and Sticky Footer
export function generateInvoice(user, transaction, customer) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // [FIX] Ensure Footer fits
            if (y + 150 > doc.page.height) { // Check if Total + Footer fits
                doc.addPage();
                y = 50;
            }
            ...
            // [FIX] Dynamic Footer Placement (Attached to content)
            y += 40;
            
            // Background box for footer
            doc.rect(0, y, 600, 60).fill('#f9f9f9');
            
            doc.fillColor(COLORS.text).fontSize(10).font(FONTS.italic)
               .text('Thank you for your business!', 50, y + 25, { align: 'center', width: 500 });
